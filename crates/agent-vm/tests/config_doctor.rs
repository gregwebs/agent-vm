...
fn reports_defaults_and_preserves_the_pre_existing_sections() {
    ...
    let codex = stdout.find("1. codex").expect("codex row");
    let opencode = stdout.find("2. opencode").expect("opencode row");
    let claude = stdout.find("3. claude").expect("claude row");
    let copilot = stdout.find("4. copilot").expect("copilot row");
    let shell = stdout.find("5. shell").expect("shell row");
    assert!(
        codex < opencode && opencode < claude && claude < copilot && copilot < shell,
        "{stdout}"
    );
