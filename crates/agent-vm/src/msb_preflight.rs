...
//! migration(s) and pointing at `agent-vm doctor --reset-msb-db` (#28),
//! before agent-vm hands the DB to msb at all — closing the loop between
//! #30 (detect) and #28 (recover). Called from both the boot path
//! (`run::launch`) and the `agent-vm msb` passthrough (`msb_cmd::run`).
...
// passthrough print identical text. Names >=1 offending migration and
// the #28 reset command.
