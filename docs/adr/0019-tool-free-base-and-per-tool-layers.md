...
- **Redeclaring a shipped tool requires its `layer`.** A `[[tools]]` entry that copies `claude` to adjust its `args` must carry `layer = { builtin = … }` (or a `path`) or a non-default tool set boots a guest without that tool — a bare command-not-found in the guest, or a `setup` diagnostic naming the image it checked. Documented in USAGE.md's config section.
