...
- `pi_credential_inspection`'s `serde_json` parsing and its duplicate-rejecting visitor (the *shape* dispatch, allocation, formatting and redaction are trusted; only the provider-id and finding-budget *decisions* are proved), and `secrets::is_known_placeholder`'s `str::as_bytes` and `ALL_PLACEHOLDERS` iteration — the scanner module is **not** formally verified;
