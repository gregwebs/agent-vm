...
The omitting snapshot is taken **under the per-fork lock** and **unioned with the launch's first measurement**, never a replacement: a fork root renamed away and recreated, an atomic `auth.json` replacement that leaves a hardlink holding the old bytes, or a `~/.pi` link re-pointed at another target would otherwise make the copier forget an inode, a route, or a Pi-home spelling the launch had positively identified. The fresh half still matters — it is what sees a fork root or credential created since that first measurement (`--mount ~/.pi:fork` with `~/.pi` created in between).

The lock, however, only coordinates fork **initializers**. It is not synchronization with host Pi writers, so a host process can rename the fork root away between the top-of-`prepare` measurement and the copier's re-measurement and recreate it — with a credential — before the copy. In that ordering the measured routes contain no entry for the root (their last existing ancestor was `$HOME`), so the *root-relative* route list alone is empty. The **static path table** is what closes it: it compares **both spellings of both sides** — the fork root's canonical path and its literal absolute spelling (a fork root is never canonicalized before the copier reads it; `expand_follow_links` skips forks) against every Pi-home spelling `measure` recorded: the configured `$HOME/.pi`, the raw `$HOME/.pi` a `--mount` carries, and each target a symlinked `~/.pi` resolved to. So `agent/auth.json` and `agent/models.json` are still omitted whether the root was renamed away before `relatives_under` or `~/.pi` is a link no measurement resolved. It supplements, and does not replace, the measured signals: a **symlinked credential file**, a hardlink under another name, or a mount alias is named by no path table entry (the table spells Pi-*home* paths, never an arbitrary alias), so a host writer who recreates such an alias between the copier's re-measurement and the copy remains a residual gap (the same class of unbounded-walk gap named in *Accepted gaps*); and the copy itself is still a window the launcher cannot hold open against a host writer.
...
Verified code calls into, and trusts: all syscalls (`fs::metadata`, `fs::symlink_metadata`, `canonicalize`, the `st_dev`/`st_ino` measurement), `std::fs::canonicalize`'s resolution semantics, `Path::strip_prefix`'s component-wise prefix test for the static fork-omission signal, the route ↔ index mapping, and the invariant that `measure` emits a route's ancestors deepest-first. `Path` → bytes is the trusted adapter; both paths are canonical (absolute, no trailing `/`, no `.`/`..`). The static signal compares every spelling a root can carry — its canonical path when it exists, and its literal absolute spelling (a fork root is deliberately never canonicalized before the copier reads it) — against every Pi-home spelling `measure` recorded: the configured `$HOME/.pi`, the raw `$HOME/.pi`, and a symlinked `~/.pi`'s target(s).
