...
The fork copier omits a node by **two signals** before creating anything at its destination — no empty file, no placeholder: the node's `fstat` identity (catching the file, a hardlink to it under any name, and a `:fork:follow-links` materialized target), and its fork-root-relative path (catching a protected file created between `measure` and the copy, which has no measured identity). The containing directory survives. The route set those signals are derived from is re-measured **under the per-fork lock**, immediately before the copy: the measure taken at the top of `prepare` cannot see a fork root that did not exist then (`--mount ~/.pi:fork` with `~/.pi` created in between), which would leave the relative-path signal empty for exactly the window it exists to cover. The residual window is the copy itself, and it is covered by the relative-path signal.
