...
- **A config error cannot be reported by clap.** That is why the failure is deferred, `allow_external_subcommands` is enabled on the broken path, and the near-miss carve-out exists.
